...
export const usePremiumCourses = (): UseUserSubscriptionCoursesResult => {
  ...
  useEffect(() => {
    const fetchUserCourses = async () => {
      try {
        ...
        const res = await fetch(`${API_URL}/api/user/courses/premium/${user.id}`, {
          headers: {
        ...
        const premiumCourseData = await res.json();

        console.log('Received premium courses data:', JSON.stringify(premiumCourseData, null, 2));

        // Format courses
        const formattedCourses = premiumCourseData.map((course: any): Course => {
