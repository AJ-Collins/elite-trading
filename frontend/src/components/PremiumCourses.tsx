...
const PremiumCourses = () => {
  const { courses, loading, error } = usePremiumCourses();
        ...
        <div className="px-4 sm:px-6 lg:px-8 w-full">
          {/* Header */}
              <div className="flex justify-between items-center mb-6">
                  <h2 className="text-xl md:text-2xl font-bold">All premium courses <span className="bg-gray-900 text-white text-xs px-2 py-1 rounded-xl ml-2">{courses.length} COURSES</span></h2>
              </div>
              {/* Error message */}
            {error && (
